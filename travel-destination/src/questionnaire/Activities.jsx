...
<div className="border rounded-lg w-[500px] p-4 mt-30 shadow-md mx-auto">
      ...
      <h2 className="mt-2 text-4xl font-bold tracking-tight text-black-600 text-center gap-0">Find  Your  Desired  Activities </h2>
        ...
        {options.map((opt) => (
          <button key={opt.id} className={`rounded-xl border p-3 shadow hover:shadow-md transition ${selectedActivities.includes(opt.id) ? "border-indigo-500" : "border-gray-200"}`}>
            ...
            <div className="relative inset-x-0 bottom-0 p-4 text-left">
                  ...
                  <p className="mt-1 line-clamp-2 text-sm text-black-300/90">{opt.subtitle}</p>
      ...
      <div className="mt-6 flex justify-center w-full"> <button onClick={handleContinue} className="rounded-xl bg-slate-800 px-8 py-5 text-sm font-semibold text-white shadow-sm transition hover:bg-indigo-500 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-5" > Next </button></div>
    </div>
  </>
);
